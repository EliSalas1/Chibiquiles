  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // ✅ Filtrar solo productos que sean llaveros
        const llaveros = allProducts.filter((p: any) =>
          p.nombre.toLowerCase().startsWith("llavero")
        );

        setProducts(llaveros);
      } catch (err) {
    ...
    <section className="py-10">
      <div className="max-w-7xl mx-auto px-4">
        ...
        {loading ? (
          ...
          <p className="text-center text-gray-600">No hay llaveros disponibles.</p>
        ) : (
          <>
